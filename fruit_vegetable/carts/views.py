...
class CartsView(View):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if 'sku_id' in request.json_obj:
            try:
                sku = GoodsInfo.objects.get(id=request.json_obj['sku_id'])
    ...
    def get_carts_datas(self, user_id):
        ...
        :param user_id:  user.id 用户的id
        :return: data = redis存的键和值 键是：字节串sku_id ，值是：字节串 {'count':xx,'selected':xxx}
        '''
        key = self.get_cache_key(user_id)
    ...
    def get_carts_list(self, user_id):
        ...
        for sku in skus:
            ...
            sku_dict['default_image_url'] = str(sku.image_url)
            sku_dict['price'] = sku.price

            # sku_sale_attr_val, sku_sale_attr_name = get_sku_name_and_values(sku.id)

            sku_dict['sku_sale_attr_name'] = []
            sku_dict['sku_sale_attr_val'] = []
    ...
    def get(self, request, username):
        user = request.myuser
        print(user.username, '111', username)
    ...
    def merge_carts(self, user_id, carts_info):
        ...
        for c_dic in carts_info:
            sku_id = int(c_dic['id'])
            sku_data = GoodsInfo.objects.get(id=sku_id)
